...
const SearchAutocomplete2 = (props) => {
    ...
    useEffect(() => {
        debounceLoadData(search2);
    }, [search2]);

    // Same example as in *SearchRoot* component
    useEffect(() => {
        setLoading(true);
        const { out2, source2 } = getAmadeusData({
            ...props.search2,
            page: 0,
            keyword,
        });
    ...
    return (
        ...
        <>
            <Autocomplete
                ...
                onChange={(e, value) => {
                    if (value && value.name) {
                        props.setSearch2((p) => ({ ...p, keyword: value.name, page: 0 }));
                        setSearch2(value.name);
                        return;
                    }
                    setSearch2("");
                    props.setSearch2((p) => ({ ...p, keyword: "", page: 0 }));
                }}
